...
void erodeDilateFilter(image_t *inputImage, image_t *outputImage, uint8_t op)
{
	...
	// TODO Algorithmus verbessern um mehrfache Suche für Pixel zu vermeiden
	for (y = 0; y < inputImage->height; ++y) {
		for (x = 0; x < inputImage->width; ++x) {  
			
			foundMatch = 0;
			// TODO Die Fenstersuche in einen HW Block extrahieren
			...
			if ((op == FILTER_ERODE && !foundMatch) || (op == FILTER_DILATE && foundMatch)) {
